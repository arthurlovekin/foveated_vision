...
class FoveationModule(nn.Module):
    ...
    """
    def __init__(self, out_width=256, out_height=256, n_fixations=1):
        """
    ...
    def forward(self,fixation,image, shape=None): 
        ...
        left_vec = torch.min(torch.max(torch.zeros_like(fixation[...,0]),fixation[...,0]*image_xshape - shape[...,0] // 2) , image_xshape - shape[...,0])
        top_vec = torch.min(torch.max(torch.zeros_like(fixation[...,0]),fixation[...,1]*image_yshape - shape[...,1] // 2) , image_yshape -shape[...,1])

        cropped =  torch.cat([TF.crop(image, top, left,width,height) for top,left,width,height in zip(list(left_vec), list(top_vec),list(shape[...,0]),list(shape[...,1]))],dim=0)
