...
class FoveationModule(nn.Module):
    ...
    def forward(self, fixation, image, shape=None):
        ...
        if self.bound_crops:
            ...
            left_vec_term = torch.max(torch.zeros_like(
                fixation[..., 0]), fixation[..., 0]*image_xshape - widths // 2)
            left_vec = torch.min(left_vec_term, image_xshape - widths)

            top_vec_term = torch.max(torch.zeros_like(
                fixation[..., 1]), fixation[..., 1]*image_yshape - heights // 2)
